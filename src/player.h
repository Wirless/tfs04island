...
class Player : public Creature, public Cylinder
{
	...
	//888	virtual void changeBoostMana(int32_t manaChange);
	...
	protected:
		...
		void updateBaseSpeed()
		{
			if(!hasFlag(PlayerFlag_SetMaxSpeed))
				baseSpeed = vocation->getBaseSpeed() + (g_config.getDouble(ConfigManager::SPEEDUP) * (level - 1));
			else
				baseSpeed = SPEED_MAX;
		}
