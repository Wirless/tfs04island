...
void NpcScript::registerFunctions()
{
	...
	lua_register(m_luaState, "getDistanceTo", NpcScript::luaGetDistanceTo);

	lua_register(m_luaState, "getNpcId", NpcScript::luagetNpcCid);
...
int32_t NpcScript::luaGetDistanceTo(lua_State* L)
{
	//getDistanceTo(uid)
...
int32_t NpcScript::luagetNpcCid(lua_State* L)
{
	//getNpcCid()
	ScriptEnviroment* env = getEnv();
