...
bool TalkAction::houseBuy(Creature* creature, const std::string&, const std::string&)
{
	...
	if(!house->isGuild())
	{
		...
		if(!IOLoginData::getInstance()->isGoldenAccount(player->getGUID()) && house->getSize() >= 25)
		{
			player->sendCancel("Your account needs to be golden account to buy a house with this size.");
			g_game.addMagicEffect(player->getPosition(), MAGIC_EFFECT_POFF);
			return false;
		}
...
bool TalkAction::houseSell(Creature* creature, const std::string&, const std::string& param)
{
	...
	if(!house->isGuild())
	{
		...
		if(!IOLoginData::getInstance()->isGoldenAccount(tradePartner->getGUID()))
		{
			player->sendCancel("Trade player does not have a golden account.");
			g_game.addMagicEffect(player->getPosition(), MAGIC_EFFECT_POFF);
			return false;
		}
		...
		if(IOGuild::getInstance()->getGuildId(guildId, param_))
		{
			if(player->isGuildInvited(guildId))
			{
				...
				if(ChatChannel* guildChannel = g_chat.getChannel(player, 0x00))
					guildChannel->talk("", MSG_CHANNEL_O, buffer);
			}
			else
