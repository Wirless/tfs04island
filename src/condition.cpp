...
bool ConditionRegeneration::executeCondition(Creature* creature, int32_t interval)
{
	...
	//{
		...
		if(internalManaTicks >= manaTicks)
		{
			...
			{
				if(getSubId() != 0)
					g_game.combatChangeMana(creature, creature, manaGain/*, manaGain*/);
				else
...
bool ConditionDamage::doDamage(Creature* creature, int32_t damage)
{
	...
	if(damage < 0 && attacker && attacker->getPlayer() && creature->getPlayer())
		damage = damage ; //damage = damage / 2; half field damage from condition need to check prolonged dmg but it decays right?
