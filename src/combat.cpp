...
ReturnValue Combat::canDoCombat(const Creature* attacker, const Creature* target, bool isAggressive)
{
	...
	if(const Player* targetPlayer = target->getPlayer())
	{
		...
		{
			checkZones = true;
			if((g_game.getWorldType() == WORLDTYPE_OPTIONAL && !Combat::isInPvpZone(attacker, target) && !attackerPlayer->isEnemy(targetPlayer)) || isProtected(const_cast<Player*>(attackerPlayer),
	...
	else if(target->getMonster())
	{
		...
		if((attackerPlayer = attacker->getPlayer()) || (attackerPlayer = attacker->getPlayerMaster()))
		{
			...
			{
				checkZones = true;
				if(g_game.getWorldType() == WORLDTYPE_OPTIONAL && !Combat::isInPvpZone(attacker, target) && !attackerPlayer->isEnemy(target->getPlayerMaster()))
					return RET_YOUMAYNOTATTACKTHISCREATURE;
			}
		}
	}
...
bool Combat::CombatManaFunc(Creature* caster, Creature* target, const CombatParams& params, void* data)
{
	...
	if(!g_game.combatChangeMana(caster, target, change/*, change*/))
		return false;
...
void Combat::addDistanceEffect(Creature* caster, const Position& fromPos, const Position& toPos, ShootEffect_t effect)
{
	if(effect == SHOOT_EFFECT_WEAPONTYPE)
	{
		...
		{
		case WEAPON_SWORD:
				effect = SHOOT_EFFECT_SWORD;
				break;

		case WEAPON_AXE:
				effect = SHOOT_EFFECT_AXE;
				break;

		case WEAPON_CLUB:
				effect = SHOOT_EFFECT_CLUB;
				break;

			case WEAPON_FIST:
				effect = SHOOT_EFFECT_LARGEROCK;
				break;
			/* WEAPON TYPE TEST HIT  WHIRLWIND THROW omg we gon have fist fighting throw XDDD*/ 
		// By Vizjus doesnt seem to work throws spear effect anyways? maybe need to change combat/spells distanceeffect?
			default:
...
void Combat::CombatFunc(Creature* caster, const Position& pos, const CombatArea* area, const CombatParams& params, COMBATFUNC func, void* data)
{
	...
	SpectatorVec list;
	g_game.getSpectators(list, pos, false, true, maxX + Map::maxViewportX, maxX + Map::maxViewportX, maxY + Map::maxViewportY, maxY + Map::maxViewportY);
