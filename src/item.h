...
class Item : virtual public Thing, public ItemAttributes
{
	public:
		...
		bool isAmuletCharge() const {return items[id].isAmuletCharge();}
		bool isRingCharge() const {return items[id].isRingCharge();}
		...
		void setItemCount(uint8_t n) {count = std::max((uint8_t)1, n);}
