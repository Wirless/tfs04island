...
bool ConfigManager::load()
{
	...
	if(!m_loaded) //info that must be loaded one time- unless we reset the modules involved
	{
		...
		m_confString[RSA_PRIME1] = getGlobalString("rsaPrime1", "12586587003411089195003170341393726670993652952302076828400035229867177025059870270014644696782943991305421536598132561463966846332385394807086505503984979");
		m_confString[RSA_PRIME2] = getGlobalString("rsaPrime2", "8218794225311588208139040606818875721629340791870228110338446023877860368810142523342593804452030691087664832285548644414079102629780418332209533377074369");
		m_confString[RSA_PUBLIC] = getGlobalString("rsaPublic", "65537");
		m_confString[RSA_MODULUS] = getGlobalString("rsaModulus", "");
		m_confString[RSA_PRIVATE] = getGlobalString("rsaPrivate", "");
	}
	...
	m_confDouble[HOWMANY_MC] = getGlobalDouble("howmanymc", 2);
	...
	m_confDouble[RATE_OFFLINE_SKILL] = getGlobalDouble("offlineRateSkill", 1);
	m_confDouble[RATE_OFFLINE_MAGIC] = getGlobalDouble("offlineRateMagic", 1);
	m_confDouble[RATE_LOOT] = getGlobalDouble("rateLoot", 1);
	m_confDouble[SPEEDUP] = getGlobalDouble("speedup", 1);
	...
	m_confNumber[FRAG_TIME] = getGlobalNumber("TimeToDecreaseFrags", 14400);
	m_confNumber[KILLS_TO_RED] = getGlobalNumber("KillsToRedSkull", 3);
	m_confNumber[KILLS_TO_BAN] = getGlobalNumber("KillsToBan", 6);
	m_confNumber[BAN_LENGTH_UNJUST] = getGlobalNumber("BanLengthUnjust", 7 * 24 * 60 * 60);
	m_confNumber[WARNINGS_TO_FINALBAN_UNJUST] = getGlobalNumber("WarningsToFinalBanUnjust", 4);
	m_confNumber[FINALBAN_LENGTH_UNJUST] = getGlobalNumber("FinalBanLengthUnjust", 30 * 24 * 60 * 60);
	...
	m_confBool[STACKABLE_RUNES]	= getGlobalBool("stackableRuneCharges", true);
	...
	m_confBool[ALLOW_OVER_SPAWN] = getGlobalBool("allowOverSpawn", false);
