...
ReturnValue Spells::onPlayerSay(Player* player, const std::string& words)
{
	...
	return g_game.internalCreatureSay(player, type, ret, player->isGhost(), NULL, &pos) ? RET_NOERROR : RET_NOTPOSSIBLE;
}
...
bool Spell::checkInstantSpell(Player* player, Creature* creature)
{
	...
	if(!isAggressive || !targetPlayer || Combat::isInPvpZone(player, targetPlayer) || player->getSkullType(targetPlayer) != SKULL_NONE)
		return true;
...
int32_t Spell::getManaCost(const Player* player) const
{
	if(player)
	{
		...
		else if(levelPercent)
			return (int32_t)std::floor((double)(player->getLevel() * levelPercent) / 100.);
	}
...
bool InstantSpell::canThrowSpell(const Creature* creature, const Creature* target) const
{
	...
	return (!(fromPos.z != toPos.z || (range == -1 && !g_game.canThrowObjectTo(fromPos, toPos, checkLineOfSight)) || (range != -1 && !g_game.canThrowObjectTo(fromPos, toPos, checkLineOfSight, range, range))));
}
...
bool InstantSpell::canCast(const Player* player) const
{
	...
	if(player->hasFlag(PlayerFlag_IgnoreSpellCheck) || (!isLearnable() && (vocSpellMap.empty() || vocSpellMap.find(player->getVocationId()) != vocSpellMap.end())))
		return true;
...
ReturnValue ConjureSpell::internalConjureItem(Player* player, uint32_t conjureId, uint32_t conjureCount, bool transform/* = false*/, uint32_t reagentId/* = 0*/)
{
	if(!transform)
	{
...
bool RuneSpell::loadFunction(const std::string& functionName)
{
	...
	if(tmpFunctionName == "chameleon")
		...
		function = Convince;
	else if(tmpFunctionName == "forceconvince")
		function = ForceConvince;
...
bool RuneSpell::ForceConvince(const RuneSpell* spell, Creature* creature, Item*, const Position&, const Position& posTo)
{
	Player* player = creature->getPlayer();
	if(!player)
		return false;
...
bool RuneSpell::executeUse(Player* player, Item* item, const PositionEx& posFrom, const PositionEx& posTo, bool, uint32_t creatureId)
{
	...
	if(result)
	{
		...
		if(g_config.getBool(ConfigManager::STACKABLE_RUNES))
		{
