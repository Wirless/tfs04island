...
//567567 manualy remove golden days
...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
	...
	<< "`posz`, `cap`, `lastlogin`, `lastlogout`, `lastip`, `conditions`, `redskulltime`, `redskull`, `skull`, `skulltime`, `guildnick`, "
	...
	if(!group || !group->getOutfit())
	{
		...
		if(outfitId > 0)
		{
			...
			wearable = Outfits::getInstance()->getOutfit(player->defaultOutfit.lookType, outfit);
	...
	if(rankId > 0)
	{
		...
		if((result = db->storeQuery(query.str())))
		{
			...
			query.str("");
			query << "SELECT `id`, `guild_id`, `enemy_id` FROM `guild_wars` WHERE (`guild_id` = " << player->guildId << " OR `enemy_id` = " << player->guildId << ")" << tmpStatus;
			if((result = db->storeQuery(query.str())))
			{
				War_t war;
				do
...
bool IOLoginData::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
	...
	if(player->redSkullTicks > 0){
		redSkullTime = std::time(NULL) + player->redSkullTicks;
	}
...
bool IOLoginData::playerDeath(Player* _player, const DeathList& dl)
{
	...
	{
		query.str("");
		query << "INSERT INTO `killers` (`death_id`, `final_hit`, `unjustified`, `war`) VALUES (" << deathId << ", " << it->isLast() << ", " << it->isUnjustified() << ", " << it->getWar().war << ")";
		if(!db->query(query.str()))
			return false;
...
bool IOLoginData::isGoldenAccount(uint32_t guid)
{
	...
	query.str("");
	query << "SELECT `golden_days` FROM `accounts` WHERE `id` = " << account << " LIMIT 1";
	if(!(result = db->storeQuery(query.str())))
		return false;

	const uint32_t goldenaccount = result->getDataInt("golden_days");
	result->free();
	return goldenaccount > 0;
}
