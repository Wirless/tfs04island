	...
	for(node = f.getChildNode(node, type); node != NO_NODE; node = f.getNextNode(node, type))
	{
		...
		switch(type)
		{
			...
			case ITEM_GROUP_TELEPORT:
				...
				break;
			case ITEM_GROUP_AMULET:
				//not used
				iType->type = ITEM_TYPE_AMULET;
				break;
			case ITEM_GROUP_RING:
				//not used
...
bool Items::loadFromXml()
{
	...
	if(!doc)
	{
		std::clog << "[Warning - Items::loadFromXml] Cannot load items file." << std::endl << getLastXMLError() << std::endl;
		return false;
	}
	...
	{
		std::clog << "[Warning - Items::loadFromXml] Cannot load randomization file." << std::endl << getLastXMLError() << std::endl;
		return false;
	}
...
void Items::parseItemNode(xmlNodePtr itemNode, uint32_t id)
{
	...
	for(xmlNodePtr itemAttributesNode = itemNode->children; itemAttributesNode; itemAttributesNode = itemAttributesNode->next)
	{
		...
		else if(tmpStrValue == "increasefire") // it should be a fail beacause it should increase the damage received and both given so need to recourse that or make it also absorb? 
		{
			if(readXMLInteger(itemAttributesNode, "value", intValue))
				it.getAbilities()->increment[COMBAT_FIREDAMAGE] += intValue;
				// or that will work as intended and this will be useless it.getAbilities()->fieldAbsorb[COMBAT_FIREDAMAGE] += intValue;
		}
		...
		//create new conditions 421421 found it simply didnt make sure that i typed the itemAttributesNode right < :) bnewb
		//421421 need declaration of these values it is not done in item.h investigate
		
		else if(tmpStrValue == "fieldabsorbpercentice")
		{
			if(readXMLInteger(itemAttributesNode, "value", intValue))
				it.getAbilities()->fieldAbsorb[COMBAT_ICEDAMAGE] += intValue;
		}
		else if(tmpStrValue == "fieldabsorbpercentholy")
		{
			if(readXMLInteger(itemAttributesNode, "value", intValue))
				it.getAbilities()->fieldAbsorb[COMBAT_HOLYDAMAGE] += intValue;
		}
