...
{
	checkDecayEvent = Scheduler::getInstance().addEvent(createSchedulerTask(EVENT_DECAYINTERVAL, boost::bind(&Game::checkDecay, this)));
	checkCreatureEvent = Scheduler::getInstance().addEvent(createSchedulerTask(EVENT_CREATURE_THINK_INTERVAL, boost::bind(&Game::checkCreatures, this)));
	checkLightEvent = Scheduler::getInstance().addEvent(createSchedulerTask(EVENT_LIGHTINTERVAL, boost::bind(&Game::checkLight, this)));
	checkWarsEvent = Scheduler::getInstance().addEvent(createSchedulerTask(EVENT_WARSINTERVAL, boost::bind(&Game::checkWars, this)));
	services = servicer;
...
bool Game::playerMoveCreature(uint32_t playerId, uint32_t movingCreatureId, const Position& movingCreaturePos, const Position& toPos, bool delay)
{
	...
	{
		SchedulerTask* task = createSchedulerTask(player->getNextActionTime(), boost::bind(&Game::playerMoveCreature, this, playerId, movingCreatureId, movingCreaturePos, toPos, true));
	...
	if(!Position::areInRange<1,1,0>(movingCreaturePos, player->getPosition()) && !player->hasCustomFlag(PlayerCustomFlag_CanMoveFromFar))
	{
		...
		{
			Dispatcher::getInstance().addTask(createTask(boost::bind(&Game::playerAutoWalk, this, player->getID(), listDir)));
			SchedulerTask* task = createSchedulerTask(std::max((int32_t)SCHEDULER_MINTICKS, player->getStepDuration()), boost::bind(&Game::playerMoveCreature, this, playerId, movingCreatureId, movingCreaturePos, toPos, true));
	...
	else if(delay)
	{
		...
		if(delayTime > 0)
		{
			SchedulerTask* task = createSchedulerTask(delayTime, boost::bind(&Game::playerMoveCreature, this, playerId, movingCreatureId, movingCreaturePos, toPos, false));
			player->setNextActionTask(task);
			return true;
		}
	}
	...
	if(player != movingCreature)
	{
		...
		if(!player->hasFlag(PlayerFlag_CanPushAllCreatures))
		{
			if(toTile->getCreatureCount() && !Item::items[movingCreature->getTile()->ground->getID()].walkStack)
			{
				player->sendCancelMessage(RET_NOTENOUGHROOM);
				return false;
			}
			...
			{
				if(field->isUnstepable() || field->isBlocking(movingCreature)|| !movingCreature->isImmune(field->getCombatType()))
				{
					player->sendCancelMessage(RET_NOTPOSSIBLE);
					return false;
				}
...
ReturnValue Game::internalMoveCreature(Creature* creature, Direction direction, uint32_t flags/* = 0*/)
{
	...
	if(direction < SOUTHWEST && creature->getPlayer())
	{
		...
		{
			if((!(tmpTile = map->getTile(Position(currentPos.x, currentPos.y, currentPos.z - 1))) || (!tmpTile->ground && !tmpTile->hasProperty(BLOCKSOLID))) && (tmpTile = map->getTile(Position(destPos.x, destPos.y, destPos.z - 1))) && tmpTile->ground && !tmpTile->hasProperty(BLOCKSOLID) && !tmpTile->hasProperty(FLOORCHANGEDOWN))
			{
				flags = flags | FLAG_IGNOREBLOCKITEM | FLAG_IGNOREBLOCKCREATURE;
				destPos.z--;
			}
		}
		else if(currentPos.z != 7 && (!(tmpTile = map->getTile(destPos)) || (!tmpTile->ground && !tmpTile->hasProperty(BLOCKSOLID))) && (tmpTile = map->getTile(Position(destPos.x, destPos.y, destPos.z + 1))) && tmpTile->hasHeight(3)) //try go down
		{
			flags = flags | FLAG_IGNOREBLOCKITEM | FLAG_IGNOREBLOCKCREATURE;
			destPos.z++;
		}
...
bool Game::playerMoveItem(uint32_t playerId, const Position& fromPos, uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint8_t count)
{
	...
	if(!player->canDoAction())
	{
		SchedulerTask* task = createSchedulerTask(player->getNextActionTime(), boost::bind(&Game::playerMoveItem, this, playerId, fromPos, spriteId, fromStackpos, toPos, count));
		player->setNextActionTask(task);
		return false;
	}
	...
	if(!player->hasCustomFlag(PlayerCustomFlag_CanPushAllItems) && (!item->isPushable() || (item->isLoadedFromMap() && (item->getUniqueId() || (item->getActionId() && item->getContainer())))))
	{
		player->sendCancelMessage(RET_NOTMOVABLE);
		return false;
	}

	const Position &mapToPos = toCylinder->getTile()->getPosition(), &playerPos = player->getPosition(), &mapFromPos = fromCylinder->getTile()->getPosition();
	...
	if(item->isHangable() && toCylinder->getTile()->hasProperty(SUPPORTHANGABLE))
	{
		...
		if(!Position::areInRange<1,1,0>(playerPos, mapToPos) && !player->hasCustomFlag(PlayerCustomFlag_CanMoveFromFar))
		{
			...
			if(fromPos.x != 0xFFFF && Position::areInRange<1,1,0>(mapFromPos, player->getPosition()) && !Position::areInRange<1,1,0>(mapFromPos, walkPos))
			{
			...
			{
				Dispatcher::getInstance().addTask(createTask(boost::bind(&Game::playerAutoWalk, this, player->getID(), listDir)));
				SchedulerTask* task = createSchedulerTask(std::max((int32_t)SCHEDULER_MINTICKS, player->getStepDuration()), boost::bind(&Game::playerMoveItem, this, playerId, itemPos, spriteId, itemStackpos, toPos, count));
				player->setNextWalkActionTask(task);
				return true;
			}
	...
	{
		if((std::abs(playerPos.x - mapToPos.x) > item->getThrowRange()) || (std::abs(playerPos.y - mapToPos.y) > item->getThrowRange()) || (std::abs(mapFromPos.z - mapToPos.z) * 4 > item->getThrowRange()))
		{
			player->sendCancelMessage(RET_DESTINATIONOUTOFREACH);
			return false;
		}
...
bool Game::playerUseItemEx(uint32_t playerId, const Position& fromPos, int16_t fromStackpos, uint16_t fromSpriteId, const Position& toPos, int16_t toStackpos, uint16_t toSpriteId)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;
	...
	if(ret != RET_NOERROR)
	{
		if(ret == RET_TOOFARAWAY)
		{
			...
			{
				Dispatcher::getInstance().addTask(createTask(boost::bind(&Game::playerAutoWalk, this, player->getID(), listDir)));
				SchedulerTask* task = createSchedulerTask(std::max((int32_t)SCHEDULER_MINTICKS, player->getStepDuration()), boost::bind(&Game::playerUseItemEx, this, playerId, itemPos, fromStackpos, fromSpriteId, toPos, toStackpos, toSpriteId));
				player->setNextWalkActionTask(task);
				return true;
			}
	...
	if(!player->canDoAction())
	{
		SchedulerTask* task = createSchedulerTask(player->getNextActionTime(), boost::bind(&Game::playerUseItemEx, this, playerId, fromPos, fromStackpos, fromSpriteId, toPos, toStackpos, toSpriteId));
		player->setNextActionTask(task);
		return false;
	}
...
bool Game::playerUseItem(uint32_t playerId, const Position& pos, int16_t stackpos, uint8_t index, uint16_t spriteId)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;
	...
	if(ret != RET_NOERROR)
	{
		if(ret == RET_TOOFARAWAY)
		{
			...
			{
				Dispatcher::getInstance().addTask(createTask(boost::bind(&Game::playerAutoWalk, this, player->getID(), listDir)));
				SchedulerTask* task = createSchedulerTask(std::max((int32_t)SCHEDULER_MINTICKS, player->getStepDuration()), boost::bind(&Game::playerUseItem, this, playerId, pos, stackpos, index, spriteId));
				player->setNextWalkActionTask(task);
				return true;
			}
	...
	if(!player->canDoAction())
	{
		SchedulerTask* task = createSchedulerTask(player->getNextActionTime(), boost::bind(&Game::playerUseItem, this, playerId, pos, stackpos, index, spriteId));
		player->setNextActionTask(task);
		return false;
	}
...
bool Game::playerUseBattleWindow(uint32_t playerId, const Position& pos, int16_t stackpos, uint32_t creatureId, uint16_t spriteId)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;
	...
	if(ret != RET_NOERROR)
	{
		if(ret == RET_TOOFARAWAY)
		{
			...
			{
				Dispatcher::getInstance().addTask(createTask(boost::bind(&Game::playerAutoWalk, this, player->getID(), listDir)));
				SchedulerTask* task = createSchedulerTask(std::max((int32_t)SCHEDULER_MINTICKS, player->getStepDuration()), boost::bind(&Game::playerUseBattleWindow, this, playerId, pos, stackpos, creatureId, spriteId));
	...
	if(!player->canDoAction())
	{
		SchedulerTask* task = createSchedulerTask(player->getNextActionTime(), boost::bind(&Game::playerUseBattleWindow, this, playerId, pos, stackpos, creatureId, spriteId));
		player->setNextActionTask(task);
		return false;
	}
	...
	player->setIdleTime(0);
	player->setNextActionTask(NULL);
	return g_actions->useItemEx(player, pos, creature->getPosition(), creature->getParent()->__getIndexOfThing(creature), item, creatureId);
}
...
bool Game::playerRotateItem(uint32_t playerId, const Position& pos, int16_t stackpos, const uint16_t spriteId)
{
	...
	if(pos.x != 0xFFFF && !Position::areInRange<1,1,0>(pos, player->getPosition()))
	{
		...
		{
			Dispatcher::getInstance().addTask(createTask(boost::bind(&Game::playerAutoWalk, this, player->getID(), listDir)));
			SchedulerTask* task = createSchedulerTask(std::max((int32_t)SCHEDULER_MINTICKS, player->getStepDuration()), boost::bind(&Game::playerRotateItem, this, playerId, pos, stackpos, spriteId));
			player->setNextWalkActionTask(task);
			return true;
		}
...
//	Container* tradeContainer = tradeItem->getContainer();
//	uint32_t howmanyougot = tradeContainer->getItemHoldingCount() + 1;
...
bool Game::playerAcceptTrade(uint32_t playerId)
{
	...
	if(ret1 == RET_NOERROR && ret2 == RET_NOERROR)
	{
		...
		if(ret1 == RET_NOERROR && ret2 == RET_NOERROR)
		{
			internalMoveItem(player, tradeItem1->getParent(), tradePartner, INDEX_WHEREEVER, tradeItem1, tradeItem1->getItemCount(), NULL, FLAG_IGNOREAUTOSTACK);
			internalMoveItem(tradePartner, tradeItem2->getParent(), player, INDEX_WHEREEVER, tradeItem2, tradeItem2->getItemCount(), NULL, FLAG_IGNOREAUTOSTACK);
...
bool Game::playerSay(uint32_t playerId, uint16_t channelId, MessageClasses type, const std::string& receiver, const std::string& text)
{
	...
	switch(type)
	{
		...
		case MSG_SPEAK_PRIVATE:
		case MSG_SPEAK_PRIVATE_RED:
		case MSG_SPEAK_ANSWER_RVR:
		...
		case MSG_CHANNEL_RVR:
			return playerReportRuleViolation(player, text);
			break;
		case MSG_CONTINUE_RVR:
			return playerContinueReport(player, text);
			break;
		case MSG_BROADCAST:
			return playerBroadcastMessage(player, MSG_BROADCAST, text, statementId);
...
bool Game::playerSpeakTo(Player* player, MessageClasses type, const std::string& receiver, const std::string& text, uint32_t statementId)
{
	...
	if(toPlayer->hasCondition(CONDITION_GAMEMASTER, GAMEMASTER_IGNORE) && !player->hasFlag(PlayerFlag_CannotBeMuted))
	{
		char buffer[70];
		if(!canSee)
	...
	if(type == MSG_SPEAK_PRIVATE_RED && !player->hasFlag(PlayerFlag_CanTalkRedPrivate))
		type = MSG_SPEAK_PRIVATE;
...
bool Game::playerReportRuleViolation(Player* player, const std::string& text)
{
	...
	for(UsersMap::const_iterator it = channel->getUsers().begin(); it != channel->getUsers().end(); ++it)
		it->second->sendCreatureChannelSay(player, MSG_CHANNEL_RVR, text, CHANNEL_RVR, rvr->time);
...
bool Game::playerContinueReport(Player* player, const std::string& text)
{
	...
	toPlayer->sendCreatureSay(player, MSG_CONTINUE_RVR, text);
...
bool Game::combatBlockHit(CombatType_t combatType, Creature* attacker, Creature* target, int32_t& healthChange, bool checkDefense, bool checkArmor, bool field/* = false*/, bool element/* = false*/)
{
	if(healthChange > 0)
		return false;
...
bool Game::combatChangeHealth(CombatType_t combatType, Creature* attacker, Creature* target, int32_t healthChange, MagicEffect_t hitEffect/* = MAGIC_EFFECT_UNKNOWN*/, Color_t hitColor/* = COLOR_UNKNOWN*/, bool force/* = false*/)
{
	CombatParams params;
...
*/ // display critical hit if the(damage > max - 0.2 )
	...
	else
	{
		...
		if(damage > 0)
		{
			if(target->hasCondition(CONDITION_MANASHIELD) && params.combatType != COMBAT_UNDEFINEDDAMAGE)
			{
				...
				elementDamage = 0; 
				if(manaDamage && combatChangeMana(attacker, target, -manaDamage,/*888 +manaDamage,*/ params.combatType, true))
			...
			if(damage > 0)
			{
					...
					if(textColor < COLOR_NONE && magicEffect < MAGIC_EFFECT_NONE)
				{
					...
					if(elementDamage)
					{
						...
						////
						if(elementDamage > damagemax -0.2)
					{
						sprintf(buffer, "%d+%d", damage, elementDamage);
						getCombatDetails(params.element.type, magicEffect, textColor);
						addMagicEffect(list, targetPos, magicEffect);
						addAnimatedText(targetPos, COLOR_DARKRED, "CRITICAL!");
					}
					else 
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, int32_t manaChange,/* int32_t manaBoostChange,*/ CombatType_t combatType/* = COMBAT_MANADRAIN*/, bool inherited/* = false*/)
{
	const Position& targetPos = target->getPosition();
	if(manaChange > 0)
	{
	...
	/*
	if(manaBoostChange > 0)
	{
		bool deny = false;
		CreatureEventList statsChangeEvents = target->getCreatureEvents(CREATURE_EVENT_STATSCHANGE);
		for(CreatureEventList::iterator it = statsChangeEvents.begin(); it != statsChangeEvents.end(); ++it)
		{
			if(!(*it)->executeStatsChange(target, attacker, STATSCHANGE_MANAGAIN, COMBAT_HEALING, manaBoostChange))
				deny = true;

		}
		if(deny)
			return false;
		target->changeBoostMana(manaBoostChange);
		if(g_config.getBool(ConfigManager::SHOW_MANA_CHANGE) && !target->isGhost() && (g_config.getBool(ConfigManager::SHOW_MANA_CHANGE_MONSTER) || !target->getMonster()))
		{
			char buffer[20];
			sprintf(buffer, "+%d", manaBoostChange);
...
bool Game::playerSharePartyExperience(uint32_t playerId, bool activate)
{
	...
	if(!player->getParty() || (!player->hasFlag(PlayerFlag_NotGainInFight) && player->hasCondition(CONDITION_INFIGHT)))
		return false;
...
bool Game::playerViolationWindow(uint32_t playerId, std::string name, uint8_t reason, ViolationAction_t action, std::string comment, std::string statement, uint32_t statementId, bool ipBanishment)
{
	...
	if(end != std::string::npos)
	{
		...
		for(StringVec::iterator it = vec.begin(); it != vec.end() && i < 4; ++it, ++i)
		{
			...
			for(StringVec::iterator tit = tec.begin(); tit != tec.end(); ++tit)
			{
				...
				if(tmp[0] != 's' && tmp[0] != 'm' && tmp[0] != 'h' && tmp[0] != 'd' && tmp[0] != 'w' && tmp[0] != 'o' && tmp[0] != 'y')
					continue;
	...
	switch(action)
	{
		case ACTION_STATEMENT:
		{
			...
			IOBan::getInstance()->addStatement(target->getGUID(), reason, comment, player->getGUID(), -1, statement);
		...
		case ACTION_NAMEREPORT:
		{
			...
			if(!IOBan::getInstance()->addPlayerBanishment(target->getGUID(), banTime, reason, action, comment, player->getGUID(), tmp))
			{
		...
		case ACTION_BANREPORT:
		{
			...
			{
				if(!IOBan::getInstance()->addNotation(account.number, reason, comment, player->getGUID(), target->getGUID()))
				{
			...
			if(!IOBan::getInstance()->addAccountBanishment(account.number, banTime, reason, action, comment, player->getGUID(), target->getGUID()))
			{
				account.warnings--;
		...
		case ACTION_BANREPORTFINAL:
		{
			...
			if(allow)
				IOBan::getInstance()->addPlayerBanishment(target->getGUID(), -1, reason, action, comment, player->getGUID(), (PlayerBan_t)g_config.getNumber(ConfigManager::NAME_REPORT_TYPE));
		...
		case ACTION_DELETION:
		{
			//completely internal
			account.warnings++;
			if(!IOBan::getInstance()->addAccountBanishment(account.number, -1, reason, ACTION_DELETION, comment, player->getGUID(), target->getGUID()))
			{
				account.warnings--;
	...
	{
		if(!length[2])
			length[2] = time(NULL) + g_config.getNumber(ConfigManager::IPBAN_LENGTH);
