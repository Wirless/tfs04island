...
bool Outfits::parseOutfitNode(xmlNodePtr p)
{
	...
	for(xmlNodePtr listNode = p->children; listNode != NULL; listNode = listNode->next)
	{
		...
		{
			std::clog << "[Error - Outfits::parseOutfitNode] Missing gender(s) for an outfit with id " << outfit.outfitId << " and looktype " << outfit.lookType << std::endl;
			continue;
		}
		...
		if(readXMLString(listNode, "storageId", strValue))
			outfit.storage = strValue;
...
std::string Outfits::getStorageId(uint32_t outfitId) {
	for(OutfitList::iterator it = allOutfits.begin(); it != allOutfits.end(); it++) {
		if(it->outfitId == outfitId) return it->storage;
	}
