...
void LuaInterface::registerFunctions()
{
	...
	lua_register(m_luaState, "setCreatureHealth", LuaInterface::luaSetCreatureHealth);

	//setCreatureMaxMana(cid, mana)
	lua_register(m_luaState, "setCreatureMaxMana", LuaInterface::luaSetCreatureMaxMana);

	//setCreatureMana(cid, mana)
	lua_register(m_luaState, "setCreatureMana", LuaInterface::luaSetCreatureMana);
	...
	//doForceConvinceCreature(cid, target)
	lua_register(m_luaState, "doForceConvinceCreature", LuaInterface::luaDoForceConvinceCreature);
	...
	lua_register(m_luaState, "getPlayerRedSkullTicks", LuaInterface::luaGetPlayerRedSkullTicks);

	//doPlayerSetRedSkullTicks(cid, amount)
	lua_register(m_luaState, "doPlayerSetRedSkullTicks", LuaInterface::luaDoPlayerSetRedSkullTicks);
...
int32_t LuaInterface::luaGetPlayerRedSkullTicks(lua_State* L)
{
...
int32_t LuaInterface::luaDoPlayerSetRedSkullTicks(lua_State* L)
{
	//doPlayerSetRedSkullTicks(cid, amount)
...
int32_t LuaInterface::luaDoCreatureAddMana(lua_State* L)
{
	...
	//888 int32_t manaBoostChange = popNumber(L);
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		if(aggressive)
			g_game.combatChangeMana(NULL, creature, manaChange/*, manaBoostChange*/);
		else
...
/*
int32_t LuaInterface::luaDoCreatureForceAddMana(lua_State* L)
{
	//doCreatureForcAddMana(uid, mana[, aggressive])
	bool aggressive = true;
	if(lua_gettop(L) > 2)
		aggressive = popBoolean(L);

	int32_t manaChange = popNumber(L);
	int32_t manaBoostChange = popNumber(L);
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		if(aggressive)
			...
			creature->changeBoostMana(manaChange);
...
int32_t LuaInterface::luaDoShowTextDialog(lua_State* L)
{
	...
	if(params > 4)
		#ifdef __FREEBSD__
		length = std::abs(static_cast<long int>(popNumber(L)));
		#else
...
int32_t LuaInterface::luaDoPlayerAddExperience(lua_State* L)
{
	...
	if(Player* player = env->getPlayerByUID(popNumber(L)))
	{
		if(amount > 0)
			...
			#ifdef __FREEBSD__
			player->removeExperience(std::abs(static_cast<long int>(amount)));
			#else
...
int32_t LuaInterface::luaDoForceConvinceCreature(lua_State* L)
{
	//doforceConvinceCreature(cid, target)
	uint32_t cid = popNumber(L);
...
int32_t LuaInterface::luaSetCreatureHealth(lua_State* L)
{
	//setCreatureHealth(uid, health)
	uint32_t Health = (uint32_t)popNumber(L);
...
int32_t LuaInterface::luaSetCreatureMana(lua_State* L)
{
	//setCreatureMana(uid, mana)
	uint32_t Mana = (uint32_t)popNumber(L);
...
/*888
int32_t LuaInterface::luaSetCreatureBoostMana(lua_State* L)
{
	//setCreatureBoostMana(uid, mana++)
	uint32_t Mana = (uint32_t)popNumber(L);

	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		creature->changeBoostMana(Mana);
		lua_pushboolean(L, true);
	}
	else
	{
