...
/*888
void Creature::changeBoostMana(int32_t manaBoostChange)
{
	if(manaBoostChange > 0)
		mana += std::min(manaBoostChange, ++mana);
	else
		mana = std::max((int32_t)0, mana + manaBoostChange);
}
*/
	...
	else if(checkDefense || checkArmor)
	{
		...
		if(checkArmor)
		{
			...
			if(armorValue > 1)
			{
				...
				maxArmorReduction = (int32_t)std::ceil(((armorValue * 0.95) - 1)/* + minArmorReduction*/);
			}
...
void Creature::removeSummon(const Creature* creature)
{
	...
	(*it)->setDropLoot(LOOT_DROP_NONE);
	(*it)->setLossSkill(false);
	(*it)->setMaster(NULL);
	(*it)->unRef();
	summons.erase(it);
}
