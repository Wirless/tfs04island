...
void Player::onUpdateTileItem(const Tile* tile, const Position& pos, const Item* oldItem, const ItemType& oldType, const Item* newItem, const ItemType& newType)
{
...
void Player::onCreatureAppear(const Creature* creature)
{
	...
	if(g_config.getBool(ConfigManager::USE_STAMINA) && lastLogout && stamina < STAMINA_MAX)
	{
		...
		if(ticks > 0)
		{
			...
			if(period > 0)
			{
				ticks = (int64_t)((g_config.getDouble(ConfigManager::RATE_STAMINA_GAIN) * period) / g_config.getDouble(ConfigManager::RATE_STAMINA_THRESHOLD));
...
void Player::onTargetChangeZone(ZoneType_t zone)
{
	...
	else if(zone == ZONE_OPEN && g_game.getWorldType() == WORLDTYPE_OPTIONAL && attackedCreature->getPlayer() && !attackedCreature->getPlayer()->isEnemy(this))
	{
...
//movement onStepTimeExhaustion	setNextAction(OTSYS_TIME() + getStepDuration(dir));
}
...
void Player::onCreatureMove(const Creature* creature, const Tile* newTile, const Position& newPos, const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	if(tradeState != TRADE_TRANSFER && ((tradeItem && !Position::areInRange<1,1,0>(tradeItem->getPosition(), getPosition())) || (tradePartner && !Position::areInRange<2,2,0>(tradePartner->getPosition(), getPosition()))))
		g_game.internalCloseTrade(this);
}
...
void Player::onUpdateInventoryItem(slots_t, Item* oldItem, const ItemType&, Item* newItem, const ItemType&)
{
	if(tradeState == TRADE_TRANSFER)
		return;
...
void Player::onThink(uint32_t interval)
{
	...
	if((timeNow - lastPong) >= 60000 && !getTile()->hasFlag(TILESTATE_NOLOGOUT) && !isConnecting && !pzLocked && !hasCondition(CONDITION_INFIGHT))
	{
		if(hasClient())
...
bool Player::isMuted(uint16_t channelId, MessageClasses type, int32_t& time)
{
	...
	return (type != MSG_CHANNEL_Y || (channelId != CHANNEL_GUILD && !g_chat.isPrivateChannel(channelId)));
}
...
void Player::removeExperience(uint64_t exp, bool updateStats/* = true*/)
{
	...
	uint64_t currLevelExp = Player::getExpForLevel(level), nextLevelExp = Player::getExpForLevel(level + 1);
	if(nextLevelExp > currLevelExp)
		levelPercent = Player::getPercentLevel(experience - currLevelExp, nextLevelExp - currLevelExp);
	else
		levelPercent = 0;
...
bool Player::onDeath()
{
	...
	{
		if(((OTSYS_TIME() - it->second.ticks) / 1000) > g_config.getNumber(ConfigManager::FAIRFIGHT_TIMERANGE))
			continue;
	...
	else if(pvpBlessing && (int32_t)std::floor((100. * pvpDamage) / std::max(1U, totalDamage)) >= g_config.getNumber(ConfigManager::PVP_BLESSING_THRESHOLD))
		usePVPBlessing = true;
		...
		case SLOT_RIGHT:
			if(item->getSlotPosition() & SLOTP_RIGHT)
			{
				...
				else if(inventory[SLOT_LEFT])
				{
					...
					//edits dual shield wielding ret = RET_NOERROR;
...
double Player::getGainedExperience(Creature* attacker) const
{
	...
	if((min > 0.0 && level < (uint32_t)std::floor(attackerLevel * min)) || (max > 0.0 && level > (uint32_t)std::floor(attackerLevel * max)))
		return 0;
...
void Player::onTarget(Creature* target)
{
	...
	if(!hasFlag(PlayerFlag_NotGainInFight))
	{
		if(target != this)
		{
			...
			{
				pzLocked = true;
				if(!isPartner(targetPlayer) && !Combat::isInPvpZone(this, targetPlayer) && !targetPlayer->hasAttacked(this) && !targetPlayer->isEnemy(this))
				{
...
void Player::onTargetDrain(Creature* target, int32_t points)
{
	...
	if(party && target && (!target->getMaster() || !target->getMaster()->getPlayer()) && target->getMonster() && target->getMonster()->isHostile()) //we have fulfilled a requirement for shared experience
...
bool Player::isEnemy(const Player* player) const
{
	...
	return !warMap.empty() && (warMap.find(guild) != warMap.end());
}
...
bool Player::onKilledCreature(Creature* target, DeathEntry& entry)
{
	...
	if(target->getMonster() && !target->isPlayerSummon() && (condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_HUNTING, g_config.getNumber(ConfigManager::HUNTING_DURATION))))
		addCondition(condition);
	...
	if(!targetPlayer->hasAttacked(this) && target->getSkull() == SKULL_NONE && targetPlayer != this && (addUnjustifiedKill(targetPlayer) || entry.isLast()))
...
bool Player::rateExperience(double& gainExp, Creature* target)
{
	...
	gainExp *= rates[SKILL__LEVEL] * g_game.getExperienceStage(level, vocation->getExperienceMultiplier());
...
/*888
void Player::changeBoostMana(int32_t manaBoostChange)
{
	if(!hasFlag(PlayerFlag_HasInfiniteMana))
		Creature::changeBoostMana(manaBoostChange);
...
bool Player::changeOutfit(Outfit_t outfit, bool checkList)
{
    ...
    std::string storage = Outfits::getInstance()->getStorageId(outfitId);
    if(storage.length() > 0) {
        std::string sval = "";
        if(!this->getStorage(storage, sval)) return false;
        int32_t intValue = atoi(sval.c_str());
        if(intValue <= 0) return false;
        sendCancel("Sorry, not possible.");
    }
	else { sendCancel("Sorry, not possible."); return false; }
...
{
	if(g_game.getWorldType() != WORLDTYPE_OPEN || hasFlag(PlayerFlag_NotGainInFight) || hasCustomFlag(PlayerCustomFlag_NotGainSkull))
		return;
...
{
	if(!g_config.getBool(ConfigManager::USE_FRAG_HANDLER) || hasFlag(PlayerFlag_NotGainInFight) || g_game.getWorldType() != WORLDTYPE_OPEN || hasCustomFlag(PlayerCustomFlag_NotGainUnjustified) || hasCustomFlag(PlayerCustomFlag_NotGainSkull))
		return false;

	if(attacked->getPlayer()->isEnemy(this))
	{
       return false;
	}
	...
	{
        Account account = IOLoginData::getInstance()->loadAccount(getAccount(), true);
		bool success = false;
		if(account.warnings >= g_config.getNumber(ConfigManager::WARNINGS_TO_FINALBAN_UNJUST)) {
    	IOBan::getInstance()->addAccountBanishment(accountId, (time(NULL) + g_config.getNumber(ConfigManager::FINALBAN_LENGTH_UNJUST)), 28, ACTION_BANISHMENT, "Unjustified player killing.", 0, guid);
			success = true;
		}
		else {
       	IOBan::getInstance()->addAccountBanishment(accountId, (time(NULL) + g_config.getNumber(ConfigManager::BAN_LENGTH_UNJUST)), 28, ACTION_BANISHMENT, "Unjustified player killing.", 0, guid);
			success = true;
		}
			...
			else if(checkText(text, "yes") && talkState[3])
			{
				if(g_config.getBool(ConfigManager::GENERATE_ACCOUNT_NUMBER))
				{
					...
					if(id)
					{
						...
						msg << "Your account has been created, you may manage it now, but please remember your account id " << managerChar << " and password " << managerString << "!";
					}
					else
...
bool Player::addOfflineTrainingTries(skills_t skill, int32_t tries)
{
	...
	if(skill == SKILL__MAGLEVEL)
	{
		...
		tries *= g_config.getDouble(ConfigManager::RATE_OFFLINE_MAGIC);
	...
	else
	{
		...
		tries *= g_config.getDouble(ConfigManager::RATE_OFFLINE_SKILL);
