...
void Monster::onThink(uint32_t interval)
{
    ...
    if(despawn()) // jesli spelnia warunki despawnu z configa, czyli ustawiony despawn radius i despawn range
    { // oba kurwa nie jedno albo drugie
        if(spawn)
        { // to
            spawn->removeMonster(this); // wyjebac go ze spawna jak by zdechl 
            spawn->startEvent(); // odpalic event spawnowania nastepnego, w czasie ustalonym na respie
            spawn = NULL; // wyjebac mu spawn
            masterRadius = -1; // wyjebac mu limit zasiegu, tak jak bys go postawil /m chuj
        }
    }
	...
	it != mType->spellAttackList.end(); ++it){
	 ...
	 if(canUseSpell(myPos, targetPos, *it, interval, inRange)){
	  if(it->chance >= (uint32_t)random_range(1, 100)){
			...
			minCombatValue = (int32_t)(it->minCombatValue);
			maxCombatValue = (int32_t)(it->maxCombatValue);

			if(this->hasCondition(CONDITION_FIGHT_DELAY) && (it->isMelee)){}
			else{ 
			 if(this->getPlayer()){ 
				 }else if(it->isMelee){ 
					Condition* condition = Condition::createCondition(CONDITIONID_MELEE, CONDITION_FIGHT_DELAY, 1500, 0); 
	...
	const spellBlock_t& sb, uint32_t interval, bool& inRange)
{
	...
	//felipe copy pasted code and send it to me for 7$???? Fixed done and dusted also in same time it fixed stairhopping bitch;)
...
bool Monster::ForceConvinceCreature(Creature* creature)
{
	Player* player = creature->getPlayer();
