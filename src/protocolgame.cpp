...
bool ProtocolGame::login(const std::string& name, uint32_t id, const std::string&, OperatingSystem_t operatingSystem, uint16_t version, bool gamemaster)
{
	//dispatcher thread
		...
		(accountManager && name == "Account Manager"))
	{
		...
		PlayerVector temp;
		temp = g_game.getPlayersByIP(player->getIP());
		if((int)temp.size() >= g_config.getNumber(ConfigManager::HOWMANY_MC)){
		disconnectClient(0x14, "Too many connections allowed from the same IP.");
		return false;
		}
	...
	if(_player->hasClient())
	{
		...
		addRef();
		m_eventConnect = Scheduler::getInstance().addEvent(createSchedulerTask(1000, boost::bind(&ProtocolGame::connect, this, _player->getID(), operatingSystem, version)));
		return true;
	}
...
void ProtocolGame::GetMapDescription(int32_t x, int32_t y, int32_t z, int32_t width, int32_t height, NetworkMessage_ptr msg)
{
...
void ProtocolGame::parseViolationWindow(NetworkMessage& msg)
{
	...
	addGameTask(&Game::playerViolationWindow, player->getID(), target, reason, action, comment, "", statementId, ipBanishment);
}
	...
	if(msg)
	{
		...
		for(RuleViolationsMap::const_iterator it = g_game.getRuleViolations().begin(); it != g_game.getRuleViolations().end(); ++it)
		{
			...
			if(rvr.isOpen && rvr.reporter)
				AddCreatureSpeak(msg, rvr.reporter, MSG_CHANNEL_RVR, rvr.text, channelId, NULL, rvr.time);
		}
	}
}
...
void ProtocolGame::sendAddCreature(const Creature* creature, const Position& pos, uint32_t stackpos)
{
	...
	for(VIPMap::iterator it = player->VIPList.begin(); it != player->VIPList.end(); ++it)
	{
		...
		if(IOLoginData::getInstance()->getNameByGuid((*it).first, vipName))
		{
			...
			sendVIP((*it).first, vipName, (tmpPlayer && player->canSeeCreature(tmpPlayer)));
		}
	}
}
...
void ProtocolGame::sendMoveCreature(const Creature* creature, const Tile*, const Position& newPos, uint32_t newStackpos, const Tile*, const Position& oldPos, uint32_t oldStackpos, bool teleport)
{
	if(creature == player)
	{
		NetworkMessage_ptr msg = getOutputBuffer();
...
void ProtocolGame::sendHouseWindow(uint32_t windowTextId, House*, uint32_t, const std::string& text)
{
	NetworkMessage_ptr msg = getOutputBuffer();
	if(!msg)
		return;
...
void ProtocolGame::sendOutfitWindow()
{
	...
	#ifdef __PROTOCOL_77__
	////msg->put<uint16_t>(player->sex % 3 ? 128 : 136 : 150); // outfit patch future reference
	 //msg->put<uint16_t>(player->isPremium() ? (player->sex % 3 ? 186 : 253 : 150) : (player->sex % 3 ? 167 : 150)); 
	msg->put<uint16_t>(player->sex % 2 ? 161 : 229); // outfit patch future reference   MALE START / FEMALE START
    msg->put<uint16_t>(player->isPremium() ? (player->sex % 2 ? 200 : 264) : (player->sex % 2 ? 167 : 235)); //  MALE FINAL PREMIUM / FEMALE FINAl PREMIUM // MALE START PREMIUM / FEMALE START PREMIUM
...
void ProtocolGame::reloadCreature(const Creature* creature)
{
	...
	TRACK_MESSAGE(msg);
	if(std::find(knownCreatureList.begin(), knownCreatureList.end(), creature->getID()) != knownCreatureList.end())
	{
...
void ProtocolGame::AddAnimatedText(NetworkMessage_ptr msg, const Position& pos, uint8_t color, const std::string& text)
{
	msg->put<char>(0x84);
	msg->putPosition(pos);
	msg->put<char>(color);
...
void ProtocolGame::AddDistanceShoot(NetworkMessage_ptr msg, const Position& from, const Position& to, uint8_t type)
{
	msg->put<char>(0x85);
...
void ProtocolGame::AddCreatureSpeak(NetworkMessage_ptr msg, const Creature* creature, MessageClasses type, std::string text, uint16_t channelId, Position* pos, uint32_t statementId)
{
	msg->put<char>(0xAA);
	#ifdef __PROTOCOL_77__
	msg->put<uint32_t>(0);
...
void ProtocolGame::MoveUpCreature(NetworkMessage_ptr msg, const Creature* creature, const Position& newPos, const Position& oldPos, uint32_t)
{
	if(creature != player)
		return;
...
void ProtocolGame::MoveDownCreature(NetworkMessage_ptr msg, const Creature* creature, const Position& newPos, const Position& oldPos, uint32_t)
{
	if(creature != player)
		return;
