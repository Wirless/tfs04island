local soulrequired = 3
	...
	min = -(maglevel*1)/2 -100
	max = -(maglevel*1)/2 -150
...
function onUseWeapon(cid, var)
	...
	local min = getPlayerMagLevel(cid)/2+100 -- should be the same as the min/max above
	local max = getPlayerMagLevel(cid)/2+150
    ...
    if getPlayerLevel(cid) >= 40 then
        ...
        doPlayerSendCancel(cid, 'You need level 40 to use this weapon.')
    end
end