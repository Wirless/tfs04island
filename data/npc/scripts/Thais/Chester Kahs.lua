...
if msgcontains(msg, 'gamel rebel') or msgcontains(msg, 'gamel is rebel') or msgcontains(msg, 'gamel is a rebel') then
	npcHandler:say("Are you saying that Gamel is a member of the rebellion?", 1)
	chesterkahs_talk_state = 1
			
elseif chesterkahs_talk_state == 1 and msgcontains(msg, 'no') then
	npcHandler:say("Then don't bother me with that. I am a busy man.", 1)
elseif chesterkahs_talk_state == 1 and msgcontains(msg, 'yes') then
	npcHandler:say("Do you know what his plans are about?", 1)
	chesterkahs_talk_state = 2
	
elseif chesterkahs_talk_state == 2 and msgcontains(msg, 'magic crystal lugri deathcurse') or chesterkahs_talk_state == 2 and msgcontains(msg, 'Magic crystal lugri deathcurse') then
	npcHandler:say("That is terrible! Will you give me the crystal?", 1)	
	chesterkahs_talk_state = 3
elseif chesterkahs_talk_state == 2 and msgcontains(msg, '') then
	npcHandler:say("Tell me precisely what he asked you to do! What, to whom, and what for! It's important!", 1)	
	chesterkahs_talk_state = 2

elseif chesterkahs_talk_state == 3 and msgcontains(msg, 'no') then
	npcHandler:say("Traitor!", 1)
	doSendMagicEffect(getCreaturePosition(getNpcId(  )), 13)
	doSendMagicEffect(getPlayerPosition(cid), 15)
	doAddCondition(cid, fire)
	chesterkahs_talk_state = 0
	npcHandler:releaseFocus()
	npcHandler:resetNpc()	
elseif chesterkahs_talk_state == 3 and msgcontains(msg, 'yes') then
	if doPlayerRemoveItem(cid, Cflifecrystal, 1) == TRUE then
	doPlayerAddItem(cid, Cflifering)
	npcHandler:say("Thank you! Take this ring. If you ever need a healing, come, bring the scroll, and ask me to 'heal'.", 1)	
	chesterkahs_talk_state = 0
	else
	npcHandler:say("Sorry, you have none.", 1)	
	chesterkahs_talk_state = 0	
	end

elseif msgcontains(msg, 'heal') then
	npcHandler:say("Do you need the healing now?", 1)
	chesterkahs_talk_state = 4
	
elseif chesterkahs_talk_state == 4 and msgcontains(msg, 'yes') then
	if doPlayerRemoveItem(cid, Cflifering, 1) == TRUE then
