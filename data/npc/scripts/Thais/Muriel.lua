...
muriel_talk_state = 814

elseif muriel_talk_state == 814 and msgcontains(msg, 'yes') or muriel_talk_state == 814 and msgcontains(msg, 'Yes') then
	if doPlayerRemoveMoney(cid, 150) == TRUE then
	doPlayerAddItem(cid, Cfspellbook, 1)
	npcHandler:say("Here you are.", 1)
	else
	npcHandler:say("Come back when you have enough money.", 1)
	end
	muriel_talk_state = 0
elseif muriel_talk_state == 814 and msgcontains(msg, '') then
