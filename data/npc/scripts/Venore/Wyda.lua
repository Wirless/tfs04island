...
function greetCallback(cid)
	if getPlayerVocation(cid) == 2 or getPlayerVocation(cid) == 6 then
		if getPlayerItemCount(cid, Cfsnakebiterod) or getPlayerItemCount(cid, Cfmoonlightrod) or getPlayerItemCount(cid, Cfvolcanicrod) or getPlayerItemCount(cid, Cfquagmirerod) or getPlayerItemCount(cid, Cftempestrod) then 
...
elseif msgcontains(msg, 'blood herb') or msgcontains(msg, 'Blood herb') then
	if getPlayerItemCount(cid, Cfbloodherb) >= 1 then
...
elseif wyda_talk_state == 1 and msgcontains(msg, 'yes') or wyda_talk_state == 1 and msgcontains(msg, 'yes') then
	if doPlayerRemoveItem(cid, Cfbloodherb, 1) == TRUE then
		...
		doPlayerAddItem(cid, Cfwitchesbroom, 1)
