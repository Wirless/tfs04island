...
keywordHandler:addKeyword({'name'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "My name is Carlson from the Nordic Tibia Ferries."})
...
keywordHandler:addKeyword({'vega'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "This island is Vega."})
...
if msgcontains(msg, "bring me to senja") then
	Carlsonbcprice = 10
	Carlsonbcdestination = Ferrysenja
	bcname = "Senja"
	if (getTilePzInfo(pos) == FALSE) then
		if doPlayerRemoveMoney(cid, Carlsonbcprice) == TRUE then
		doTeleportThing(cid, Carlsonbcdestination)
		doSendMagicEffect(Carlsonbcdestination, 10)
...
elseif msgcontains(msg, "bring me to tibia") then
	Carlsonbcprice = 0
	Carlsonbcdestination = Ferrycarlin
	bcname = "Tibia"
	if (getTilePzInfo(pos) == FALSE) then
		if doPlayerRemoveMoney(cid, Carlsonbcprice) == TRUE then
		doTeleportThing(cid, Carlsonbcdestination)
...
elseif msgcontains(msg, "bring me to folda") then
	Carlsonbcprice = 10
	Carlsonbcdestination = Ferryfolda
	bcname = "Folda"
	if (getTilePzInfo(pos) == FALSE) then
		if doPlayerRemoveMoney(cid, Carlsonbcprice) == TRUE then
		doTeleportThing(cid, Carlsonbcdestination)
...
elseif msgcontains(msg, 'senja') and npcHandler.focus == cid then
	...
	Carlsonbcdestination = Ferrysenja
	bcname = "Senja"
	npcHandler:say("Do you want a round-trip passage to ".. bcname .." for ".. Carlsonbcprice .." gold?", 1)
	TalkState(cid, 94982)
...
elseif GetTalkState(cid) == 94982 and msgcontains(msg, 'yes') and npcHandler.focus == cid then
	if (getTilePzInfo(pos) == FALSE) then
		if doPlayerRemoveMoney(cid, Carlsonbcprice) == TRUE then
		doTeleportThing(cid, Carlsonbcdestination)
		doSendMagicEffect(Carlsonbcdestination, 10)
