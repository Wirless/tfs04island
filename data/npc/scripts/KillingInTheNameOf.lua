                ...
                if getCreatureStorage(cid, KILLSSTORAGE_BASE + id) >= tasks[id].killsRequired then
                    for _, reward in ipairs(tasks[id].rewards) do
                        ...
                        if isInArray({REWARD_MONEY, "money"}, reward.type:lower()) and not deny then
                            doPlayerAddMoney(cid, reward.value[1]*MONEYMULTIPLIER)
                        elseif isInArray({REWARD_EXP, "exp", "experience"}, reward.type:lower()) and not deny then
                            doPlayerAddExperience(cid, reward.value[1]* EXPMULTIPLIER)
							doSendAnimatedText(getPlayerPosition(cid), "" .. reward.value[1]* EXPMULTIPLIER .. "", TEXTCOLOR_WHITE)
							doPlayerSendTextMessage(cid, MESSAGE_EVENT_ADVANCE, "You gained " .. reward.value[1]* EXPMULTIPLIER .. " experience points.")
