...
if msgcontains(msg, "bring me to tibia") then
	Andersonbcprice = 0
	Andersonbcdestination = Ferrycarlin
	bcname = "Tibia"
	if getPlayerSkullType(cid) <= 2 then
		if doPlayerRemoveMoney(cid, Andersonbcprice) == TRUE then
		doTeleportThing(cid, Andersonbcdestination)
		doSendMagicEffect(Andersonbcdestination, 10)
...
elseif msgcontains(msg, "bring me to folda") then
	Andersonbcprice = 10
	Andersonbcdestination = Ferryfolda
	bcname = "Folda"
	if getPlayerSkullType(cid) <= 2 then
		if doPlayerRemoveMoney(cid, Andersonbcprice) == TRUE then
		doTeleportThing(cid, Andersonbcdestination)
...
elseif msgcontains(msg, "bring me to vega") then
	Andersonbcprice = 10
	Andersonbcdestination = Ferryvega
	bcname = "Vega"
	if getPlayerSkullType(cid) <= 2 then
		if doPlayerRemoveMoney(cid, Andersonbcprice) == TRUE then
		doTeleportThing(cid, Andersonbcdestination)
...
elseif msgcontains(msg, "folda") and npcHandler.focus == cid then
	...
	Andersonbcdestination = Ferryfolda
	bcname = "Folda"
	npcHandler:say("Do you want a round-trip passage to ".. bcname .." for ".. Andersonbcprice .." gold?", 1)
	TalkState(cid, 94981)
...
elseif GetTalkState(cid) == 94981 and msgcontains(msg, 'yes') and npcHandler.focus == cid then
		if doPlayerRemoveMoney(cid, Andersonbcprice) == TRUE then
		doTeleportThing(cid, Andersonbcdestination)
		doSendMagicEffect(Andersonbcdestination, 10)
		npcHandler:say("Have a nice trip!", 1)
