...
npc = getCreaturePosition(getNpcId(  ))
--Make the npc wont send players to the same boat--
if msgcontains(msg, "carlin") and npc.x == 32360 then
	npcHandler:say("what if.. I told you... that we're already in Carlin?..", 1)
	eric_talk_state = 0
	
elseif msgcontains(msg, "mistrock") and npc.x == 32002 then
	npcHandler:say("what if.. I told you... that we're already in Mistrock?..", 1)
	eric_talk_state = 0

elseif msgcontains(msg, "") and getPlayerVocation(cid) <= 4 then
	npcHandler:say("I'm sorry, I only serve premium players!", 1)
	eric_talk_state = 0
	
elseif msgcontains(msg, "") and getQuestPoints(cid) < 10 then
	npcHandler:say("I'm sorry, but you don't seem experienced enough for this trip! get 10 quest points and come back and I'll gladly help you!", 1)
	eric_talk_state = 0
...
elseif msgcontains(msg, 'mistrock') and npcHandler.focus == cid then
	...
	bcdestination = TravelserviceMistrock
	bcname = "Mistrock"
	npcHandler:say("Do you seek a passage to ".. bcname .." for ".. bcprice .." gold?", 1)
