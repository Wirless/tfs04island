...
npcHandler:say("If you are that curious, do you want to buy a key for 5000 gold? Don't blame me if you get sucked in.", 1)
elathruek_talk_state = 1033

elseif elathruek_talk_state == 1033 and msgcontains(msg, 'yes') or elathruek_talk_state == 1033 and msgcontains(msg, 'Yes') then
	if doPlayerRemoveMoney(cid, 5000) == TRUE then
	HELLGATEKEY = doPlayerAddItem(cid, 2089, 1)
	doSetItemActionId(HELLGATEKEY, 3012)
	npcHandler:say("Here you are.", 1)
	else
	npcHandler:say("Come back when you have enough money.", 1)
	end
	elathruek_talk_state = 0
elseif elathruek_talk_state == 1033 and msgcontains(msg, '') then
	npcHandler:say("Believe me, it's better for you that way.", 1)
	elathruek_talk_state = 0
