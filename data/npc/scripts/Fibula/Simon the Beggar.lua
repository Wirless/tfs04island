...
if msgcontains(msg, 'help') or msgcontains(msg, 'Help') then
	npcHandler:say("I need gold. Can you spend me 100 gold pieces?", 1)
	TalkState(cid, 1)
elseif GetTalkState(cid) == 1 and msgcontains(msg, 'yes') or GetTalkState(cid) == 1 and msgcontains(msg, 'Yes') then
	...
	npcHandler:say("Thank you very much. Can you spend me 500 more gold pieces? I will give you a nice hint.", 1)
	TalkState(cid, 2)
	else
	npcHandler:say("You've not enough money for me.", 1)
	TalkState(cid, 0)
	end
...
elseif GetTalkState(cid) == 2 and msgcontains(msg, 'yes') or GetTalkState(cid) == 2 and msgcontains(msg, 'Yes') then
	...
	npcHandler:say("That's great! I have stolen something from Dermot. You can buy it for 200 gold. Do you want to buy it?", 1)
	TalkState(cid, 3)
	else
	npcHandler:say("Sorry, that's not enough.", 1)
	TalkState(cid, 0)
	end	
elseif GetTalkState(cid) == 2 and msgcontains(msg, '') then
	npcHandler:say("It was your decision.", 1)
	TalkState(cid, 0)

elseif GetTalkState(cid) == 3 and msgcontains(msg, 'yes') or GetTalkState(cid) == 2 and msgcontains(msg, 'Yes') then
	if doPlayerRemoveMoney(cid, 200) == TRUE then
	DEEPERFIBULA = doPlayerAddItem(cid, 2087, 1)
	doSetItemActionId(DEEPERFIBULA, 3940)
	npcHandler:say("Now you own the hot key.", 1)
	TalkState(cid, 3)
	else
	npcHandler:say("Pah! I said 200 gold. You don't have so much.", 1)
	TalkState(cid, 0)
	end		
elseif GetTalkState(cid) == 2 and msgcontains(msg, '') then
	npcHandler:say("Ok. No problem. I'll find another buyer.", 1)
	TalkState(cid, 0)
