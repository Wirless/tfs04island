...
elseif msgcontains(msg, 'sparaeaek') or msgcontains(msg, 'phoeaeaeaenix') then
	npcHandler:say("The spark of the phoenix is given by me and by the great geomancer of the local earthtemple. Do you wish to receive my part of blessing of the phoenix for 40.000 gold?", 1)
	pydar_talk_state = 1394


elseif pydar_talk_state == 1394 and msgcontains(msg, 'yes') then
	if getCreatureStorage(cid, 1339) >= 1 then
		if doPlayerRemoveMoney(cid, 40000) == TRUE then
			if doPlayerAddBlessing(cid, 5) == true then
...
elseif pydar_talk_state == 1394 and msgcontains(msg, '') then
