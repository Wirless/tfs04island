...
function isItemChargeAmulet(itemid)
	local item = getItemInfo(itemid)
	return item and item.type == ITEM_TYPE_AMULET or false
end

function isItemChargeRing(itemid)
	local item = getItemInfo(itemid)
	return item and item.type == ITEM_TYPE_RING or false
end
