...
local function exerciseDummyTrainEvent(params, weapon)
    if isPlayer(params.cid) then
        ...
        if getDistanceBetween(playerPosition, params.currentPos) == 0 and getPlayerSlotItem(params.cid, CONST_SLOT_RIGHT).itemid == params.itemid then
            ...
            if weaponCharges >= 1 then
				...
				return false
					...
					doPlayerAddSkillTry(params.cid, SKILL_SHIELD, (skillTries * params.dummy.skillRate) * getConfigValue("rateSkill")/ 2)
            ...
            doPlayerSendTextMessage(params.cid, MESSAGE_EVENT_ADVANCE, "You have finished your training.The weapon is not in your right hand!")
        end
		else
		doPlayerSendTextMessage(params.cid, MESSAGE_EVENT_ADVANCE, "The weapon is not in your right hand! Do not switch weapons or relog to reset trainng system.")
		return false
    end
...
function onUse(cid, item, fromPos, target, toPos, isHotkey)
    ...
    if not getPlayerExerciseTrain(cid) then
        ...
        params.dummy = dummy
        exerciseDummyTrainEvent(params, weapon)
		doPlayerSendTextMessage(params.cid, MESSAGE_EVENT_ADVANCE,"You started Training")
        else
