...
--Wheat--
if isInArray(WHEATITEM, item.itemid) == 1 then
	if isInArray(MILL, item2.itemid) == 1 then
...
------
--Flour--	
elseif isInArray(FLOURITEM, item.itemid) == 1 then
	if isInArray(ITEMCONTAINER, item2.itemid) == 1 and item2.type == 1 then
	doChangeTypeItem(item2.uid, 0)
	doTransformItem(item.uid, 2693)
	elseif isInArray(OVENOFF, item2.itemid) == 1 then
	doPlayerSendTextMessage(cid,22,"The oven must be on, else you can't bake.")
	end
------	
--Dough--
elseif isInArray(DOUGHITEM, item.itemid) == 1 then
	if isInArray(OVENON, item2.itemid) == 1 then
